...
---
# Critique Color
**Project note (VexelTech):** the palette and every certified pair are in `vexel-tech-site/DESIGN.md` **Colors** and **Components**; the accent is five variables, not one — `--c-accent-text-dark` (machine yellow as text on asphalt, 9.46:1), `--c-accent-text-light` (deep amber as text on light, 5.43:1), `--c-accent-ground` (the one primary call per frame, nothing else), `--c-accent-draw-dark` / `--c-accent-draw-light` (strokes) — `--c-machine-yellow` is never referenced by a component, and machine yellow on white (1.88:1) is a failure, never a judgement call.
