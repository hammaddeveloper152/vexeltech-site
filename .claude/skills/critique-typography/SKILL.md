...
---
# Critique Typography
**Project note (VexelTech):** the scale is `vexel-tech-site/DESIGN.md` **Typography** ("The whole page in the loud register") as tokens in `src/styles/tokens.css` — `--t-loud-fig` / `--t-loud-h` / `--t-loud-s` / `--t-loud-small` (Monigue, uppercase, line-height `--t-loud-line` 0.9), `--t-quote-size`, `--t-h2` / `--t-h3` (Clash, controls only), `--t-body` / `--t-small` / `--t-label` (Satoshi); the hero display in `Hero.css` is the one deliberate non-token size, and the type floor is 11px per `BUILD-LAW.md`, so judge against those tables rather than the generic thresholds below.
