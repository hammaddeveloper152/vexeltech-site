...
for (const [w, h] of [[1280, 800], [390, 844]]) {
  ...
  for (const [route, sel, label, objSel, open] of FRAMES) {
    ...
    let s = 0, n = 0, os = 0, on = 0;
    const inObj = (x, y) => boxes.some((r) => x >= r[0] && x < r[2] && y >= r[1] && y < r[3]);
    for (let y = 0; y < im.height; y++) for (let x = 0; x < im.width; x++) {
      const i = (y * im.width + x) * 4;
      const hit = solid(im.data[i], im.data[i + 1], im.data[i + 2]);
      if (hit) s++;
      n++;
      if (inObj(x, y)) { on++; if (hit) os++; }
    }
    const objPct = on ? (os / on * 100) : 0;
    console.log(`  ${label.padEnd(24)} frame ${(s / n * 100).toFixed(2)}%   on the objects' own surface ${objPct.toFixed(2)}%   ${objPct <= 5 ? 'ok' : 'OVER'}`);
    await p.close();
  }
}
await b.close();
